...
; =====================================
__PRGRAM_OFFSET__ .set $1950 ; BHOP's reserved area starts after Tile_Mem (see ram_prg.inc)
.macro BHPRGRAM name, size
    .if (__PRGRAM_OFFSET__ + (size)) > ($1950 + $123)
        .error "PRGRAM overflow: name"
    .endif
    name = PRGRAM_BASE + __PRGRAM_OFFSET__
